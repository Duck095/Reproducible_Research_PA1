...
```
```{r}
conn <- DBI:: dbConnect(RSQLite:: SQLite(),"C:\\Users\\EmBeLenTHeo\\Downloads\\chinook.db")
#ktra
if (!is.null(conn)) {
  message("ok") 
} else {
...
invoices_data <- dbGetQuery(conn, invoices_query)
...
```{r}
albums_query <- "SELECT * FROM invoices"
...
```{r}
#in gtri trung lap
duplicated_value <- albums_data$Albums[duplicated(albums_data$AlbumsId)]
...
```
Top 10 nsi co nhieu albums nhat
```{r}
query <- "
SELECT 
  artists.ArtistId,
...
```
```{r}
artist_album_counts <- dbGetQuery(conn, query)
head(artist_album_counts)
```
```{r}
top_artists <- artist_album_counts %>%
  top_n(10, AlbumCount) %>%
  arrange(desc(AlbumCount))
top_artists
```
histogram phan so album cua nsi
```{r}
ggplot(artist_album_counts, aes(x= AlbumCount)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "white") +
  labs(
    title = " pbo sl album giua cac nsi",
    x = " sl album",
    y = " sl nsi"
  ) + 
  theme_minimal()
...
conn <- dbConnect(RSQLite :: SQLite(), "C:\\Users\\EmBeLenTHeo\\Downloads\\chinook.db")
monthly_data_query <- "
...
GROUP BY Month
ORDER BY Month"
monthly_data <- dbGetQuery(conn, monthly_data_query)
print(head(monthly_data))
#tinh toan theo doanh thu luy ke
monthly_data$RunningTotal <- cumsum(monthly_data$Revenue)
...
monthly_data <- monthly_data %>%
  mutate(
    Month = gsub(" ", "", Month),           # bỏ khoảng trắng: "2009 - 01" -> "2009-01"
    Month = as.Date(paste0(Month, "-01"),   # thêm ngày đầu tháng
                    format = "%Y-%m-%d")
  )
# bieu do
ggplot(monthly_data, aes(x = Month, y = RunningTotal)) +
  geom_line(color = "red", size = 1.2) +
  geom_point(color = "darkred", size = 2) +
  labs(
    title = " doanh thu luy ke theo thang",
    x = "thang",
    y = " tong doanh thu"
  )+
  theme_minimal()
...
ggplot(monthly_data, aes(x = Month)) +
  geom_col(aes(y = Revenue),fill = "skyblue", alpha = 0.6) +
  geom_line(aes(y = RunningTotal),color = "darkred", size = 1.2) +
  labs(
    title = " doanh thu luy ke theo thang2",
    x = "thang",
    y = " tong doanh thu"
  )+
  theme_minimal()
```
tim kiem xu huong trend analysis
```{r}
...
```{r}
print(invoices_data)
#tao cot moi billing local ket hop data 2 cot
invoices_data <- invoices_data %>%
  # mutate() them cot hoac thay doi cot trong df
  # paste() ghep cac gtri tu cac cot
  mutate(BillingLocal = paste(BillingAddress, BillingCity, sep = ", "))
invoices_data
```
30.them cot
```{r}
# them cot note vao bang invoice
dbExecute(conn, " ALTER TABLE invoices ADD COLUMN Ghi_Chu TEXT")

#ktra du lieu sau khi add
results <- dbGetQuery(conn, "SELECT * FROM invoices")
print(results)
```
31.doi ten
```{r}
...
#ktra cau truc sau khi xoa cot 
results <- dbGetQuery(conn, "PRAGMA table_info('invoices')")
print(results)
```
33. tinh total cho cot
```{r}
invoices_data <- tbl(conn, "invoices")

#tinh sum cho cot total theo nhom
```
{r}
#dong ket noi
...
total_query <- "SELECT * FROM Total where Total > 15"
total_data <- dbConnect(conn, total_query)
